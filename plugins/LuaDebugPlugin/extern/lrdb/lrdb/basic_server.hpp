...
class basic_server {
 ...
 protected:
    ...
    for (auto& s : callstack) {
      ...
      if (s.source()) {
		  ...
		  if (source.substr(0, 2) == "@/") {
			  ...
			  source = "@" + cwd.string() + "/" + source.substr(2);
			  //source = "@C:/dev/itgmania/" + source.substr(2);
		}
  ...
  bool get_global_request(response_message& response,
	...
	lua_State* L = LUA->Get();
	response.result =
        debugger_.get_global_table(L, depth + 1);  //+ 1 is global table self
	LUA->Release(L);
