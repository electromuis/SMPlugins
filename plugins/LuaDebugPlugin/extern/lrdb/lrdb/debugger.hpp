...
class debugger {
 public:
  ...
  debugger() :  pause_(true), step_type_(STEP_ENTRY) {}
  ...
  json::value get_global_table(lua_State* L, int object_depth = 1) {
    lua_pushglobaltable(L);
    json::value v = utility::to_json(L, -1, object_depth);
 ...
 public:
  void sethook(lua_State* L) {
    ...
    lua_sethook(L, &hook_function,
                LUA_MASKCALL | LUA_MASKRET | LUA_MASKLINE, 0);
  }
  void unsethook(lua_State* L) {
    if (L) {
      lua_sethook(L, 0, 0, 0);
      lua_pushlightuserdata(L, this_data_key());
      lua_pushnil(L);
  ...
  static bool is_file_path_match(const char* path1, const char* path2) {
    ...
    while (true) {
		...
		if (is_path_separator(*path1) && is_path_separator(*path2)) {
			path1++;
			path2++;
			continue;
		}
